...
export function correctionScore(successRate: number, correctedCount: number, totalQuestions: number): number {
  if (totalQuestions <= 0 || correctedCount <= 0) return 0;
  const correctionDepth = correctedCount / totalQuestions;
...
export async function calculateSuccessRate(
  ...
  for (const [category, { bankIds, bankNames }] of categoryMap) {
    ...
    const correctedCount = everWrongQuestions.filter((q) => q.laterCorrect).length;
    ...
    categoryResults.push({
      ...
      rawValues: {
        ...
        correctionRate: everWrongQuestions.length > 0 ? Math.round((correctedCount / everWrongQuestions.length) * 100) : (questionAttempts.length > 0 ? 100 : 0),
        correctionCoverage: totalQuestions > 0 ? Math.round((correctedCount / totalQuestions) * 100) : 0,
      },
      score: total,
    });
  }
